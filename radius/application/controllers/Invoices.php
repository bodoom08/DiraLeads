...
class Invoices extends MOBO_User
{
    ...
    function paymentsuccess() {
        ...
        // $invoice_id = 'db35e7164033e6bdd35ff771e8a7e3e46aca72';
        if(empty($invoice_id))
            show_404();
        ...
        if(empty($invoice_dtls))
            ...
            $invoice = $this->M_invoices->getById($invoice_dtls->invoice_id, $invoice_dtls->user_id);
            $package = json_decode($invoice->package);
            $price = $package->package_details->price;
            $user_info = $this->db->where('id', $invoice_dtls->user_id)->get('users')->row();
            // print_r($user_info);
