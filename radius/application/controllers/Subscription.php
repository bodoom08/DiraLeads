...
class Subscription extends MOBO_User
{
        ...
        $data['data'] = array_map(
            function ($row) {
                ...
                if(strtotime(date('Y-m-d')) > strtotime($row['end_date'])) { // only enable renew button if package expired
                    $actions .= '<button type="button" class="btn btn-outline-success" name="edit-btn" title="Renew Package" onclick="renew(\'' . $row['id'].'\'' . ','. '\'' . $row['user_id'] .'\')" data-info='.htmlspecialchars(json_encode(["ref_id" => $row['id'] ]), ENT_QUOTES, 'UTF-8').'>Renew</button>';
                }
                else {
                    $actions .= '<button type="button" class="btn btn-outline-info" name="edit-btn" title="Edit Area Info" onclick="editarea(\'' . $row['id'].'\'' . ','. '\'' . $row['user_id'] .'\')" data-info='.htmlspecialchars(json_encode(["ref_id" => $row['id'] ]), ENT_QUOTES, 'UTF-8').'>Edit Area Info</button><button type="button" class="btn btn-outline-success" name="edit-btn" title="Renew Package" onclick="renew(\'' . $row['id'].'\'' . ','. '\'' . $row['user_id'] .'\')" data-info='.htmlspecialchars(json_encode(["ref_id" => $row['id'] ]), ENT_QUOTES, 'UTF-8').'>Renew</button><button type="button" class="btn btn-outline-primary" name="edit-btn" title="Modify Package" onclick="edit(\'' . $row['id'].'\'' . ','. '\'' . $row['user_id'] .'\')" data-info='.htmlspecialchars(json_encode(["ref_id" => $row['id'] ]), ENT_QUOTES, 'UTF-8').'>Modify</button>';
                ...
                $user_info = '<ul>';
                $user_info .= '<li><b>Name: </b>'.$row['name'].'</li>';
                $user_info .= '<li><b>Email: </b>'.$row['email'].'</li>';
                $user_info .= '</ul>';
    ...
    function manage_package_area_info() {        
        ...
        $data['user_pref'] = $this->db->select(
            'notification_phone, notification_phone_no, notification_email, notification_email_id, notification_fax, notification_fax_no, notification_frequence')->where('id', $userid)->get('users')->row();
        $data['areas'] = $this->M_preferences->getAllAreas();
        ...
        $user_package_pref = $this->db
                                    ...
                                    ->where(['user_package_ref_id' => $package_table_id, 'user_id' => $userid])
                                    ->order_by('id', 'asc')
