...
class M_subscription extends CI_Model
{
    ...
    public function getThisSubscribePackage($package_id, $userid) {
        $lastPackage = $this->db
    ...
    public function getLastPackageById($package_id, $userid) {
        $lastPackage = $this->db
                ->where('package_id', $package_id)
                ->where('user_id', $userid)
                ->order_by('id', 'desc')
    ...
    public function getdata()
    {
        ...
        $this->db->select("a.id, a.package_id, a.package_name, a.validity, a.no_of_area, a.price, a.start_date, a.end_date, u.name, u.email, u.id user_id")
                        // ->where('a.user_id', $_SESSION['id'])
                        ->from('user_packages a')
                        ->group_start()
                        ->where('a.start_date<=', date('Y-m-d'))
                        ->or_where('a.end_date>=', date('Y-m-d'))
                        ->group_end()
                        ->join('users u', 'a.user_id = u.id', 'INNER');
