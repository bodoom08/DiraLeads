...
class M_pricing extends CI_Model
{
    public function getConfiguredPackage() {
        ...
        if(intval($total) != intval($tot_price) && ($action == 'new') && (intval($dis_amnt) == 0)) {            
    ...
    public function subscribe_package($invoiceId, $userid)
    {
        ...
        if ($invoice) {
            ...
            $users_packages_data = [
                ...
                'discount' => $subscribe_info->dis_amnt,
            ...
            $subscribe_info = json_decode($this->input->post('subscribe_info'));
            if($subscribe_info->action == 'modify') {
                $prev_total = $this->db->select('price, discount, area_price')
                ...
                // Add New Discount Amount With the Prev Discount Amount
                $users_packages_data['discount']    +=  $prev_total->discount; 
                $users_packages_data['price']       =   $prev_total->price + $subscribe_info->total; 
                $users_packages_data['area_price']  =   $prev_total->area_price + $subscribe_info->subtotal; 
                ...
                $this->db->where('id', $userid)->update('users', ['subscribe_flag' => 'yes']);
                ...
                if($subscribe_info->action == 'renew') {
                    ...
                    $subscribe_pref_record = $this->db
                                ...
                                ->where(['user_package_ref_id' => $record_id, 'user_id' => $userid])
