...
class Pricing extends CI_Controller
{
    ...
    public function pricing_pref() {
        ...
        $user_packages = $this->db
                                    ->select('no_of_area')
                                    ->where(['package_id' => $subscribe_record->package_selected_id, 'user_id' => $_SESSION['id']])
                                    ->order_by('id', 'desc')
                                    ->limit(1)
                                    ->get('user_packages')
                                    ->row();
        ...
        $user_package_pref = $this->db
                                    ...
                                    ->where(['user_package_id' => $subscribe_record->package_selected_id, 'user_id' => $_SESSION['id']])
                                    ->order_by('id', 'desc')
    ...
    public function custom_pricing_json_data() {
        ...
        $data = [
            ...
            'renewtext' => '<p>Validity of this package is from '.$fromdate." to ".$todate.', if you want to edit/modify this package click<button type="button" class="btn btn-link" onclick="packModify('.$package_id.')">Modify</button> </p>'
        ];
        if(empty($subscribed_package)) {
            unset($data['textdiv']);
