...
<?php /*defined in examplemyclass.php*/
    class MySql{
        ...
        function __construct($host="localhost", $dbname="diraleads", $user="root", $pass=""){
            $this->user = $user;
            $this->pass = $pass;
            $this->dbname = $dbname;
            $this->host = $host;
        ...
        */
        ...
        private function writeUTF8filename($filenamename,$content){  /* save as utf8 encoding */
            ...
            try {
                ...
                $filepath = realpath(getcwd().'/dbbkup');
                if(!file_exists($filepath)) {
                    mkdir('dbbkup');
                    var_dump('asasasa');
                    // $myfile = fopen(getcwd().'dbbkup/'.$filenamename, "w+") or die("Unable to open file!");
                    // mkdir(realpath(getcwd().$filenamename));
                }

                $dirpath = realpath(getcwd().'/dbbkup');
                $filepath = getcwd().'/dbbkup/'.$filenamename;
                if(file_exists($dirpath)) {
                    $myfile = fopen($filepath, "w+") or die("Unable to open file!");

                }

                // // var_dump(fopen($filepath, 'w+'));
                $f=fopen($filepath,"w+"); 
                // // # Now UTF-8 - Add byte order mark 
                // // var_dump($filenamename);
                // // var_dump($content);
