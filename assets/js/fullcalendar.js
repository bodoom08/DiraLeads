    ...
    for (i = 0; i < complexOptions.length; i++) {
      ...
      for (j = 0; j < chain.length; j++) {
        ...
        if ($.isPlainObject(val)) {
          ...
          // a non-null non-undefined atomic option
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function getClientRect(el) {
    ...
    var left =
      offset.left + getCssFloat(el, "border-left-width") + scrollbarWidths.left;
    var top =
      offset.top + getCssFloat(el, "border-top-width") + scrollbarWidths.top;
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  newMomentProto.week = newMomentProto.weeks = function (input) {
    var weekCalc = (this._locale || this._lang)._fullCalendar_weekCalc; // works pre-moment-2.8
  ...
  newMomentProto.isSame = function (input, units) {
    ...
    if (units) {
      ...
      return (
        oldMomentProto.isSame.call(this, input) &&
  ...
  setUTCValues = allowValueOptimization
    ? function (mom, a) {
  ...
  setLocalValues = allowValueOptimization
    ? function (mom, a) {
        // simlate what moment's accessors do
        mom._d.setTime(
  ...
  var DragListener = (fc.DragListener = Class.extend({
      ...
      if (!this.isListening) {
        ...
        if (ev && this.options.scroll) {
          ...
          if (!scrollParent.is(window) && !scrollParent.is(document)) {
            ...
            this.scrollHandlerProxy = debounce(
              proxy(this, "scrollHandler"),
              100
            );
    ...
    updateScroll: function (ev) {
      ...
      if (bounds) {
        ...
        bottomCloseness =
          (sensitivity - (bounds.bottom - ev.pageY)) / sensitivity;
        leftCloseness = (sensitivity - (ev.pageX - bounds.left)) / sensitivity;
        rightCloseness =
          (sensitivity - (bounds.right - ev.pageX)) / sensitivity;
  ...
  var CellDragListener = DragListener.extend({
    ...
    listenStart: function (ev) {
      ...
      if (ev) {
        ...
        if (subjectEl && this.options.subjectCenter) {
          ...
          if (this.origCell) {
            subjectRect =
              intersectRects(this.origCell, subjectRect) || subjectRect; // in case there is no intersection
          }
    ...
    cellOver: function (cell) {
      ...
      this.trigger(
        "cellOver",
        cell,
        isCellsEqual(cell, this.origCell),
        this.origCell
      );
    },
  ...
  var MouseFollower = Class.extend({
    ...
    start: function (ev) {
      if (!this.isFollowing) {
        ...
        $(document).on(
          "mousemove",
          (this.mousemoveProxy = proxy(this, "mousemove"))
        );
      }
    },
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var Grid = (fc.Grid = RowRenderer.extend({
    ...
    setRange: function (range) {
      ...
      this.colHeadFormat =
        view.opt("columnFormat") || this.computeColHeadFormat();
    ...
    getCell: function (row, col) {
      ...
      if (col == null) {
        if (typeof row === "number") {
          // a single-number offset
          col = row % this.colCnt;
          row = Math.floor(row / this.colCnt);
        } else {
          // an object with row/col properties
          col = row.col;
          row = row.row;
        }
      }
      ...
      var dragListener = new CellDragListener(this.coordMap, {
        ...
        listenStop: function (ev) {
          if (dayClickCell) {
            view.trigger(
              "dayClick",
              _this.getCellDayEl(dayClickCell),
              dayClickCell.start,
              ev
            );
          }
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  Grid.mixin({
      ...
      $.each(
        {
          ...
          mousedown: function (seg, ev) {
            if (
              ...
              view.isEventResizable(seg.event)
            ) {
              _this.segResizeMousedown(
                seg,
                ev,
    ...
    segDragMousedown: function (seg, ev) {
      ...
      var dragListener = new CellDragListener(view.coordMap, {
        ...
        cellOver: function (cell, isOrig, origCell) {
          ...
          if (
            dropLocation &&
            !calendar.isEventRangeAllowed(dropLocation, event)
          ) {
            disableCursor();
            dropLocation = null;
          }
        ...
        cellOut: function () {
          // called before mouse moves to a different cell OR moved out of all cells
          view.destroyDrag(); // unrender whatever was done in renderDrag
          mouseFollower.show(); // show in case we are moving out of all cells
          dropLocation = null;
        },
        cellDone: function () {
          // Called after a cellOut OR before a dragStop
          enableCursor();
        },
        dragStop: function (ev) {
    ...
    computeExternalDrop: function (cell, meta) {
      ...
      if (
        !this.view.calendar.isExternalDropRangeAllowed(
          dropLocation,
          meta.eventProps
        )
      ) {
        ...
        cellOver: function (cell, isOrig, origCell) {
          ...
          if (resizeLocation) {
            ...
            // no change? (TODO: how does this work with timezones?)
              ...
              resizeLocation.start.isSame(event.start) &&
              resizeLocation.end.isSame(eventEnd)
            ) {
              resizeLocation = null;
            }
          }
    ...
    getSegClasses: function (seg, isDraggable, isResizable) {
      ...
      ].concat(event.className, event.source ? event.source.className : []);
    ...
    getEventSkinCss: function (event) {
      ...
      return {
        ...
        "border-color":
          ...
          optionColor,
        color:
          event.textColor || source.textColor || view.opt("eventTextColor"),
      };
    },
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function compareSegs(seg1, seg2) {
    return (
      ...
      seg2.event.className.length - seg1.event.className.length ||
      seg2.event.className[0].length - seg1.event.className[0].length
    );
  }
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var DayGrid = Grid.extend({
    ...
    computeColHeadFormat: function () {
      if (this.rowCnt > 1) {
        // more than one week row. day numbers will be in each cell
        return "ddd"; // "Sat"
    ...
    computeCellDate: function (cell) {
      var colCnt = this.colCnt;
      var index =
        cell.row * colCnt + (this.isRTL ? colCnt - cell.col - 1 : cell.col);
    ...
    renderFill: function (type, segs, className) {
      ...
      if (segs.length) {
        ...
        for (col = 0; col < segCols.length; col++) {
          ...
          if (colSegs.length) {
            containerEl = $(
              '<div class="fc-' + className + '-container"/>'
            ).appendTo(tdEl);
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  TimeGrid.mixin({
    ...
    renderFgSegs: function (segs) {
      ...
      this.el.append(
        (this.eventSkeletonEl = $('<div class="fc-content-skeleton"/>').append(
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  var View = (fc.View = Class.extend({
    ...
    clear: function () {
      // clears the view of *content* but not the skeleton
      if (this.isDisplayed) {
        this.unselect();
        this.clearEvents();
        this.triggerDestroy();
  ...
  var Calendar = (fc.Calendar = fc.CalendarBase = Class.extend({
    ...
    buildViewSpecButtonText: function (spec, requestedViewType) {
      ...
      spec.buttonTextOverride =
        queryButtonText(this.overrides) || spec.overrides.buttonText; // constructor-specified buttonText lookup hash takes precedence // `buttonText` for view-specific options is a string
  ...
  function Calendar_constructor(element, overrides) {
    ...
    t.humanizeDuration = function (duration) {
      return (duration.locale || duration.lang)
        .call(duration, options.lang) // works moment-pre-2.8
        .humanize();
    };
    ...
    function _calcSize() {
      ...
      if (typeof options.contentHeight === "number") {
        ...
        suggestedViewHeight =
          options.height -
          (headerElement ? headerElement.outerHeight(true) : 0);
      } else {
    ...
    function getAndRenderEvents() {
      if (
        ...
        isFetchNeeded(currentView.start, currentView.end)
      ) {
        fetchAndRenderEvents();
      } else {
        renderEvents();
  ...
  fc.datepickerLang = function (langCode, dpLangCode, dpOptions) {
    ...
    if ($.datepicker) {
      ...
      $.datepicker.regional[dpLangCode] = $.datepicker.regional[
        langCode
      ] = dpOptions; // alias
  ...
  fc.lang = function (langCode, newFcOptions) {
    ...
    if (newFcOptions) {
      fcOptions = langOptionHash[langCode] = mergeOptions(
        fcOptions,
        newFcOptions
      );
    }
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function EventManager(options) {
    ...
    $.each(
      (options.events ? [options.events] : []).concat(
